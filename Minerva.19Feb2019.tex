...
\newcommand{\fbr}{\mathsf{ForBouqReg}}
\newcommand{\bbr}{\mathsf{BackBouqReg}}
\newcommand{\fsc}{\mathsf{FavSurCon}}
...
\newcommand{\tot}{t_{1,2}}
\newcommand{\btone}{{\bf{t}}_1}
\newcommand{\bttwo}{{\bf{t}}_2}
\newcommand{\formerE}{C}
\newcommand{\rcon}{r_0}
\newcommand{\para}{Q}
\newcommand{\Cstrong}{E}
...
\newcommand{\fa}{x_0}
\newcommand{\hit}{H}
\newcommand{\scaledle}{\mathsf{Nr}\mc{L}}
...
\newcommand{\barxmin}{\overline{\rm Corner}^{\mfl,\mc{F}}}
\newcommand{\barymin}{\overline{\rm Corner}^{\mfr,\mc{F}}}
%\newcommand{\barymin}{\bar{Y}^{{\rm min}}}
\newcommand{\qmin}{Q^{\mc{F}^1}}
%^{{\rm min}}}
\newcommand{\barqmin}{\bar{Q}^{\mc{F}^1}}
%^{{\rm min}}}
\newcommand{\test}{T}
\newcommand{\mfl}{\ell}
\newcommand{\mfr}{r}
...
\newcommand{\Rmac}{R}
\newcommand{\rmac}{r}
\newcommand{\conseqmac}{D}
\newcommand{\constn}{C'}
\newcommand{\coninit}{\Psi}
\newcommand{\condee}{\hat{D}}
\newcommand{\conbrac}{\hat{C}}
\newcommand{\Cnew}{\tilde{C}}
\newcommand{\Cbig}{C^*}
\newcommand{\Ctbd}{C_+}
\newcommand{\Ctbs}{C_-}
...
\newcommand{\wlp}{{\rm WLP}}

\newcommand{\canopynumber}{\mathsf{Canopy}{\#}}

\newcommand{\cannum}{{\#}\mathsf{SC}}

\newcommand{\boundgood}{\mathsf{G}}
\newcommand{\lshift}{\mc{L}^{\rm shift}}
\newcommand{\deltapi}{\theta}
\newcommand{\rootneigh}{\mathrm{RNI}}
\newcommand{\rootneighuse}{\mathrm{RNI}}
\newcommand{\manycan}{\mathsf{ManyCanopy}}
\newcommand{\specialpt}{\mathrm{spec}}

\newcommand{\dist}{\vert\vert}
\newcommand{\fik}{\mc{F}_i^{[K,K+1]^c}}
\newcommand{\mcfa}{\mc{H}[\fa]}
\newcommand{\tent}{{\rm Tent}}
\newcommand{\goodk}{\mc{G}_{K,K+1}}
...
\newcommand{\chimac}{\chi}
\newcommand{\xnmac}{z_{\mathcal{L}}}
\newcommand{\Cwb}{E_0}
\newcommand{\initcond}{\mathcal{I}}
\newcommand{\neargeod}{\mathsf{NearGeod}}
\newcommand{\polyunique}{\mathrm{PolyUnique}}
\newcommand{\latecoal}{\mathsf{LateCoal}}
\newcommand{\nolatecoal}{\mathsf{NoLateCoal}}
\newcommand{\normalcoal}{\mathsf{NormalCoal}}
\newcommand{\regfluc}{\mathsf{RegFluc}}
\newcommand{\mdeltaweight}{\mathsf{Max}\Delta\mathsf{Wgt}}
...
\newcommand{\down}{\mathsf{Fall}}
\newcommand{\up}{\mathsf{Rise}}


\newcommand{\paradelta}{\Delta^{\cup}\,}
%\newcommand{\paradelta}{\Delta^{{\rm para}}}



\title[Local random growth and Gibbs resamplings]{Universality in models of local random growth via Gibbs resamplings}
...
\section{Lecture One: Exponents, curvature and Gaussianity via resampling}

Important stochastic processes -- such as Brownian morion or the Airy$_2$ process --
...
We may identify these exponents by studying how closely $X$ follows the global contour offered by $\mc{C}_X$. The process $\mc{C}_X$ has a graph, a closed subset of $\R^2$, which is comprised of the union of a counterpart collection of closed planar line segments. The facet through zero may be defined to be the planar segment among these that passes through the vertical axis through the origin. The facet length $F$ is the random  length of the interval occupied by $x$-coordinates of points in the facet through zero. 
...
$$
This the scale of $\ell$ may be identified by equating the two effects:
$$
...
See Figure with caption indicating area. Let $\gamma \in \Lambda$. Then the length $L(\gamma)$ of $\gamma$
...
$$
 \PP_\lambda \Big( \textrm{$\Gamma$ has length $n$} \Big) = Z^{-1} \big( 2\lambda^{-1} \big)^n \, ,
$$
where we write $\Gamma$ for the sampled path.
This equality is a condition of exponential tail decay because $2 \lambda^{-1} < 1$.
...
\item and ${\rm ExcessArea}(\Gamma)$ typically also has order $N$; or more precisely, 
...
The answer is simple enough (see Figure): this piece-to-be-added has the distribution of a uniformly chosen downright path from $a$ to $b$.
...
We pick vertices $a$ and $b$ uniformly and independently along $\Gamma$. 
...
\end{equation}
To see this, consider the path $\Gamma'$ which is formed by the replacement of the section of $\Gamma$ about $a$ and $b$ by the planar line segment that connects these two endpoints. On the event $\mathsf{GoodHit}$, we have that $A\big(\Gamma^{{\rm in}}\big) \leq A\big(\Gamma'\big)$ -- indeed, in this circumstance, $a$ and $b$
...
Our principal result about the area-trap path model, Theorem~\ref{t.areatrap},  makes two assertions. First, mean facet length is said to have a typical order of $N^{2/3}$; and second, mean local roughness is said to have order $N^{1/3}$. We have explained the reason for the upper bound on mean facet length. We will not discuss much the accompanying lower bound, nor the bounds needed on local roughness -- beyond saying that all these bounds arise in essence from the Gaussian fluctuation that is inherent in the area-constrained path, a path which is largely free from cares dictated by area on spatial scale below $N^{2/3}$.

We have seen how resampling the area trap path model has betrayed its characteristic KPZ features -- the two exponents; its global curvature; and its local Gaussianity. The model has thus largely served the purpose of exposition that we have intended for it. It is perhaps worth mentioning, however, that the basic resampling technique of proof leads to stronger conclusions. We may define ${\rm MFL}(\Gamma)$ to be the maximum facet length of a downright path $\Gamma \in \Lambda$ -- the maximum length of any of the planar line segments that comprise the graph of  the concave majorant $\mc{C}_\Gamma$. Further we may denote by ${\rm MLR}$ the maximum local roughness of $\Gamma$ -- this being the maximum over vertices in $\Gamma$ of the distance from the vertex to the graph of $\mc{C}_\Gamma$. 
...
\subsection{Brownian last passage percolation [LPP]}





\subsubsection{The model's definition.}
...
For $x > 0$, we set $M_n^1(x) = M^1_{(0,1) \to (x,n)}$, so that $M_n^1:[0,\infty) \to \R$ is a random energy profile of geodesics emanating from the origin to a variable location.

\subsubsection{A geometric view: staircases.}\label{s.staircases}
...
\subsection{Mutually avoiding staircase collections -- and ensembles of energy profiles}
...
Note that $L_n(1,n)$ is the geodesic energy for the journey $(0,1) \to (x,n)$. How does this random variable behave for hign $n$? Since $L_n(1,n)$ is equal in law to ${\rm DysonBM}_n(1,n)$, a connection of Dyson Brownian motion to random matrix theory offers a means of furnishing an answer.

\begin{theorem}
Let $t > 0$. The point process indicated in the [Figure], namely $\big\{ {\rm DysonBM}_n(i,t): i \in \intint{n} \big\}$, is equal in law to the set of eigenvalues $\big\{ \lambda_n(i,t): i \in \intint{n}  \big\}$ of a matrix sampled from the Gaussian unitary ensemble with entry variance $t$.
\end{theorem}
...
is due to Ledoux and the second to Aubrun in view of the connections between GUE and the Dyson Brownian motion marginal, and bwtween Dyson Brownian motion and Brownian LPP. \qed

As the notation $\mc{L}^{\rm sc}_{n} \big( 1,\cdot \big)$ suggests, we want to specify a stochastic process $\mc{L}^{\rm sc}_{n} \big( 1,\cdot \big)$ that describes the scaled energy of a geodesic from the origin to a further endpoint that is in a variable location. [Figure. Caption: the parallelogram on the left is mapped to the square on the right by an affine change of coordinates. The size of the parallelogram -- its height of order $n^{1/3}$ and its width of order $n^{2/3}$ -- are, in view of Proposition~\ref{p.scaledonepoint} and Brownian scaling, suitable for the specification of the scaled ensemble as the image curve collection on the right.] 

In view of the consideration described in the figure, we define the {\em scaled} Brownian last passage percolation line ensemble
$$
...
In the figure left, the depicted geodesic, running from $(0,1)$ to $(n,n)$, has energy of the form $2n + \Theta(1) n^{1/3}$, where the random quantity $\Theta(1)$ is this geodesic's {\em scaled energy} or {\em weight}.
This weight is $\mc{L}^{\rm sc}_{n} (1,0)$. If instead, as in the figure middle, the geodesic runs from $(0,1) \to (n + 2n^{2/3},n)$, then the associated weight is $\mc{L}^{\rm sc}_{n}(1,1)$. In the case of the triple of disjoint staircases seen in figure right, the weight is $\sum_{i=1}^3 \mc{L}^{\rm sc}_{n} (i,0)$.
...
$$
which is the polymer whose journey is between the scaled coordinate locations $(0,0)$ and $(x,1)$, may be viewed as having weight  $\mc{L}^{\rm sc}_{n}(1,x)$; while the multi-polymer in the figure has weight  $\sum_{i=1}^3 \mc{L}^{\rm sc}_{n} (i,0)$.

\subsection{The Brownian Gibbs property}
...
From the figures, we see that the scaled ensemble  $\mc{L}^{\rm sc}_{n}$:
\begin{itemize}
...
is $(\rsc,\rsC)$-regular.
 ...
 Now 
$\mc{L}_n^{\scal}$ 
 enjoys integrable features -- for example, there are determinantal expressions for probabiliities such as that of the event depicted in the figure, in which each of the very short vertical intervals is visited by one of the curves in the ensemble. Prahofer and SPohn in 2002 introduced what they called the multi-line Airy process -- in essence, the high-$n$ limit of the ensemble $\mc{L}_n^{\scal}$ when the limit is taken in the sense of finite dimensional distributions. 
...
$\mc{L}: \N \times \R \to \R$ (with $\N = \{1,2\cdots,\cdots\}$) that satisfies the Brownian Gibbs property.
...
Indeed, setting $\mc{A}(i,x) = 2^{1/2} \mc{L}(i,x) + x^2$, $\mc{A}: \N \times \R \to \R$ is an ensemble that is stationary in $x$ -- this is the AIry line ensemble. 
 \end{theorem} 
 We will not explain the proof, but the basic diea is to use finite dimensional convergence, with the Brownian Gibbs property furnishing the necessary regularity. We will explain now at least a little of the technical aspect of this convergence. Consider a Brownian Gibbs resampling of 
$\mc{L}_n^{\scal}$, i.e., a random map sending the law 
$\mc{L}_n^{\scal}$ 
 to itself in which say the top two surces of this ensebmle are resampled on the interval $[-1,1]$. Figure.
...
A principal consequence of this main theorem of Corwin-H. 2014 is:
...
Let $\mc{L}: \N \times \R \to \R$.

{\bf Proof of Theorem~\ref{t.mainconsec}.} This is in essence a consequence of $\mc{L}$  being a Brownian Gibbs ensemble that is ordered, since this forces the acceptance probability on $\intint{k} \times I$ to be almost surely positive. \qed 

This conclusion was recently strengthened and quantified an extensive analysis using the Brownian Gibbs property.
...
$$ 
for all measurable subsets $A \subseteq \mc{C}_{0,0} \big( [a,b], \R \big)$ (and where $B = B(1,\cdot)$) implies that
$$
 \PP \big( \mc{L}_n^{\scal}(1,\cdot) \in A  \big) < \e \, .
$$ 
This is in essence the state of affairs achieved by Corwin-H. 2014 -- uniformity of comparison between ensemble curves and Brownian motion is achieved in the curve index $n$, but without any quantitative relation being demonstrated between the parameters $\e$ and $\delta$.
...
{\em 1.} Comparison is made of the standard birdge-valued $\mc{L}_n^{[K,K+1]}$ to Brownian bridge. The theorem does not attempt caomparsion of $\mc{L}_n(1,\cdot) - \mc{L}_n(1,K): [K,K+1] \to \R$ to standard Brownian motion on $[K,K+1]$.
...
Now the second main theorem: see Figure.
\begin{theorem}[$k$-curve closeness at one point]
...
\begin{theorem}\label{t.locfluc}[Local Fluctuation]
...
\end{theorem}
The two route to our two main theorems can be presented as a journey of three stages, labelled `basic', `intermediate' and `advanced'. See Figure.

As we turn to attempting to expalin the proof of the principal result Theorem~\ref{t.bbr} on Brownian bridge regularity, we will follow this three-stage journey. This second lecture ends by discussing the first step -- the basic elements of the Brownian Gibbs approach.

\subsection{Stage One: Brownian Gibbs basics}
...
\item C: No Big Max
\item D: Near parabolic invariance of regular eensembles
...
We want to record two forms of stochastic monotonicity regarding this law. The first concerns the replacement of the left or right boundary vectors $\bar{x}$ or $\bar{y}$ by a new vector whose components are all at least as high as their counterparts in the replaced vector. The effect is to send the law upwards, in the sense that the original law and the new one may be coupled together so that the new curves in the new law are always at least as high as their counterparts in the original.
...
\begin{proposition}\label{p.nobigmax}[No Big Max] 
Let $\mc{L}_n$ be an $n$-curve regualr ensemble.  For $r \in \big[0,\rsc/2 \cdot n^{1/9}\big]$, $t \in \big[ 2^{7/2} , 2 n^{1/3} \big]$ and $n \geq (2c)^{-18}$,
$$
...
In summary, the resampled process' value at $1$ exceeds $t/12$ with probability at least 
$$
...
$$
  \PP \Big(  \mc{L}_n(1,1) \geq t/12 \Big) \leq C \exp \big\{ - c (t/12)^{3/2} \big\} \, .
$$
Thus we find that 
$$
 \PP \Big( \mathsf{High}(t \Big) \leq   \PP \big(\mc{L}_n(1,2) <  - t/3 \big) + 2C \exp \big\{ - c (t/12)^{3/2} \big\} \leq \Theta(1) \exp \big\{ - \Theta(1) (t/12)^{3/2} \big\} \, .
$$
...
The role of this near parabolic invariance is to permit us to take a result proved near the origin, such as the No Big Max Proposition~\ref{p.nobigmax} -- and to extend its domain of validity to a much wider region of $x$- coordinates -- to this $x$ at or below scale $n^{1/9}$.
...
$$ 
We will not explain how this induction works -- but it is a BG resample argument. We will need this lower curve control in the intermediate and advanced steps, as we progress towards the proof ofthe Brownian bridge regularity  Theorem~\ref{t.bbr}. In fact the recorded bound may be available from determinantal or other integrable techniques. 


\section{Lecture 3}

This lecture is devoted to proving the Local Fluctuation Theorem~\ref{t.locfluc}. This is the second, {\em intermediate}, stage in the three-stage journey to the principal Brownian bridge regularity Theorem~\ref{l.bbr}.
The intermediate step involves introducing a new resampling of a resample, a little more subtle than the basic BG resampling -- the objects we introduce, which will be used again in the final lecture, are {\em missing closed middle reconstructed} and the associated {\em Wiener candidate}.
...
\end{itemize}
Why these names? Suppose that an experimenter realizes the process $B$ according to the law $\mc{B}_{1;x,y}^{[a,b]}$, and records the outcome in the form of retained and lost data. Suppose then that the experiment discards the lost data, and hands only the retained data to an observer, who is aware that $B$ has been sampled according to $\mc{B}_{1;x,y}^{[a,b]}$. The observer has all information except the lost data.
...
\subsection{Reprising this for real -- with a regular ensemble}

We will with the ensemble $\mc{L}: \N \times R \to \R$ for simplicity. Missing closed middle reconstruction is the reconstruction of $\mc{L}$ which is counterpart to the Brownian bridge case just discussed. 
...
The observer's point of view is depicted in the figure. Here, the observer realizes the law $\mc{B}_{1;u,v}^{[-2T,2T]}$
with $u = \mc{L}(1,-2T)$ and $v = \mc{L}(1,2T)$, where note that these $(u,v)$ values are indeed known to the observer.
The Wiener candidate $W$ is the marginal on $[\ell,r]$ of the realized process. Using the bead locations thus dictated, the two side interval process values 
$$
\mc{L}(1,\cdot): [-2T,\ell] \cup [r,2T] \to \R
$$
are obtained by affine translation as discussed before.

{\em However}, the proposed outcome must be checked for avoidance constraints between the top and the second curve on the side intervals and on the middle interval. In this way, there are {\em three tests} that must be passed: these are associated to the left side, the middle, and the right side. See Figure. See Figure. See Figure.
...
$$
First consult Figure for an explanation of the form of the left and right side tests.
...
For this application of missing closed middle reconstruction, we make the parameter settings $T = 2$, $\ell = -1$ and $r=1$.
...
\end{enumerate} 
\begin{lemma}\label{l.fav}[The favourable event is typical]
$$
...
\begin{lemma}\label{l.wcsuccess}[Wiener candidate success lower bound]
$$
...
\end{equation}
Any such $W$ is successful (provided that $\PP_\mc{F}$) because 
$$
...
Why this answer? See figure and caption.
...
See Figure. The movement of $W$ on $[0,\e]$ takes the form 
$$
  O(t) \e + \e^{1/2} G \, ,
$$
where $G$ is a standard Gaussian random variable. With $t \leq K$ and $K \e \leq \e^{1/2}$ (which is equvalent to the harmlessly supposed upper bound on $\e$, $\e \leq K^{-2}$), the movement in question is at most $2 \e^{1/2} G$. Thus we confirm the given answer.
...
$$
 ...
 \PP \Big( \mathsf{BigFluc}\big(\mc{L}\big) \Big) \cap F_t \big) + 
 \PP \big( F_t^c \big) \, .  
$$
Now,
$$
...
\section{Lecture Four: proving Brownian bridge regularity via the jump ensemble}
...
$$
\end{theorem}
Note that we are attempting a proof of this result only for curve index $k=1$ -- not much more than a notational convenience. We work merely with the spatial interval $[0,1]$
...
These incompatible choices show that the present method cannot work -- at least not with its parameters choices $T=1$, $\ell = -1$ and $r=1$.
...
now has a duration $(\log \e^{-1})^\alpha$ in which to make the jump, rather than merely unit-order time. However, the height of the hill has increased, because the attempted high jump begins at lower locations than before, in view of parabolic curvature. See Figure.
Indeed, $\mc{L}(1,\pm 2T) \approx - \big(\log \e^{-1} \big)^{2\alpha}$.
...
where $D > 0$ is a constant to be chosen suitably in applications. 
 ...
 We continue to work with a favourable event $\mathsf{Fav} = F_t$, with $t = \Theta(1) big( \log \e^{-1} \big)^{2/3}$, with $F_t$ now specified analogously to before for the new choice of $T$. See figure. The upper bound on the probability of failure of the favourable event now takes the form:
 \begin{lemma}\label{l.favour}
 $$
  \PP \big(  \mathsf{Fav}^c \big) \leq \e^{O(1) D^3} \, .
  $$
 \end{lemma}
Next, we set the parameters $\ell$ and $r$. The choice respective choices are $-T$ and $T$, but we will make a choice $\ell \in [-T,0]$ and $r \in [0,T]$ that renders the lower boundary condition $\mc{L}(2,\cdot)$ slightly more regular on $[\ell,r]$. 
...
\subsection{More promising than the Wiener candidate -- the jump ensemble}
...
We will vary the Wiener candidate $W:[\ell,r] \to \R$ to obtain the jump curve $J:[\ell,r] \to \R$. The success probability of $J$ will be at least $\e^{o(1)}$ for $\mc{F}$-data verifying $\mathsf{Fav}$ as desired. See Figure.
...
and $v = \mc{L}(1,2T)$.
We say that the Wiener candidate passes the {\em jump} test if each of its curves clears all the pole tops, namely if 
$$
\wien(x) > \mc{L}_n\big(2,x \big) \ \, \, \textrm{for all $x \in \pole$} \, .
$$
This test is weak than the middle interval test which entails the above bound for all $x \in [\ell,r]$ (and we have $P \subseteq [\ell,r]$).

Indeed, the examination of the Wiener candidate may represented n three steps:
\begin{itemize}
...
Under $\PP_\mc{F}$, the jump curve $J:[\ell,r] \to \R$ is constructed so that it has the conditional distribution of $W:[\ell,r] \to \R$ given that $W$ passes Test~$1$ and~$2$.
We may picture the jump curve as being a halfway house between the unadultered Brownian randomness of the Wiener candidate $W$
...
\begin{proposition}\label{p.jpromiserealized}
We have that
$$
...
\subsection{Proving Theorem~\ref{t.bbr} via Proposition~\ref{p.jpromiserealized}}
...
\begin{eqnarray*}
 ...
 & = &  \E \Bigg[ \frac{\PP_\mc{F} \Big( J^{[0,1]} (\cdot) \in A \Big)}{\PP_\mc{F} \Big( J \, \, \textrm{passes Test $3$} \Big)} {\bf 1}_{\mathsf{Fav}}  \Bigg] + \PP \big( \mathsf{Fav}^c \big) \, .
\end{eqnarray*}
...
\begin{proposition}\label{p.bbrj}[Brownian regularity for $J$]
...
What hope is there for the curve $J$ to succeed, i.e., to pass the middle interval test: $J(x) \geq \mc{L}(2,x)$ for all $x \in [-T,T]$. See Figure. See Figure.
...
The density at $x$ for this random excess equals (and see the left sketch!)
$$
...
$$
where recall that $T = D \big( \log \e^{-1} \big)^{1/3}$. This cost is the unacceptably high term that we saw in the high jump difficult -- in the explanation of why the intermediate, Wiener candidate, approach was inadequate. 
...
exceeds its minimum possible value by an order of  $\big( \log \e^{-1} \big)^{1/2}$ behaves as  $\exp \big\{ - \big( \log \e^{-1} \big)^{2/3} \big\}$ -- this is because the argument of the exponential $-Tx$ has this form in view of  $T = D \big( \log \e^{-1} \big)^{1/3}$  and  $x =\big( \log \e^{-1} \big)^{1/3}$.

What happens in the circumstance that this excess indeed has this order? See Figure. Note that $J$ is above ${\rm Tent}$ at $-T$, $0$ and $T$ by order 
...
For $x \in [-T,T] = [\ell,r]$,
  ...
  $$
\end{lemma}
See Figure for a proof sketch.

Since $T \approx  \big( \log \e^{-1} \big)^{1/3}$, we see that, with suitable selection of constant factors, $J$ has in the circumstance of Figure [] positive conditional probability to exceed $\mc{L}(2,\cdot)$ on $[-T,T]$ -- and thus to pass Test~$3$, the middle interval test, and be successful. 
That is,
$$
 \PP_\mc{F}  \Big( J \, \, \textrm{succeeds}\Big)  
\leq  \exp \Big\{  - O(1) \big( \log \e^{-1} \big)^{2/3} \Big\} {\bf 1}_{\mathsf{Fav}}  
  \, ,
$$
as we sought to show in deriving Proposition~\ref{p.jpromiserealized}. \qed

\subsection{Sketch of proof of Proposition~\ref{p.bbrj}}
...
Again, we work with a special case to focus our attenting: $P = \big\{ - T , 0 , T \big\}$, $\mc{L}(2,-T) = \mc{L}(2,T) = - T^2$ and $\mc{L}(2,0) = T^2$. See Figure.

Hoes does $J$ on $[-1,1]$ differ from ordinary Brownian motion on this interval. More to the point: how does $J^{[-1,1]}$ differ from standard Brownian bridge?

Let's make this comparison after fixing $J(-T)$ and $J(T)$ at the lowest possible locations that these values may adopt: $J(-T) = J(T) = -T^2$. This in essence gives a worst case scenario, in which the jump that $J$ faces is as high as it ever may be -- see the preceding figure. See also Figure.
...
Given our caricature of $J^{[-1,1]}$, which event is liable to maximize the $J^{[-1,1]}$-probability among those events whose probability under $\mc{B}_{1;0,0}^{[-1,1]}$?
...
What then is $\PP \big( J^{[-1,1]}(0) \geq R \big)$ when $R$ is so chosen? It is (and see figure!)
$$
 \exp \big\{ - (R - T)^2 \big\} \, = \,  
